...
{#if data.databaseMissing}
  <p class="notice">
    The guestbook database is not configured, so no entries can be listed.
  </p>
{/if}
...
<p class="pagination">
  ...
  {:else}
    <span>Previous</span>
  {/if}
  Page {data.page} of {data.pageCount} ({data.total} entries, {site.adminPageSize}
  per page)
  {#if data.hasNext}
