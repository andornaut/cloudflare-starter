...
export const actions: Actions = {
	default: async ({ request, cookies, platform }) => {
		...
		if (!secret) {
			...
			return fail(500, { error: 'ADMIN_SECRET is not configured on this deployment' });
		}
		...
		cookies.set(SESSION_COOKIE, await signSession(secret, expiresAt), SESSION_COOKIE_OPTIONS);
		redirect(303, '/admin');
	}
};
