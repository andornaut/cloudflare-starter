...
describe("paginate", () => {
  it("reports both ends", () => {
    expect(paginate(250, 1, 50)).toMatchObject({
      hasNext: true,
      hasPrev: false,
    });
    expect(paginate(250, 3, 50)).toMatchObject({
      hasNext: true,
      hasPrev: true,
    });
    expect(paginate(250, 5, 50)).toMatchObject({
      hasNext: false,
      hasPrev: true,
    });
  });
  ...
  it("does not add a phantom trailing page when the total divides evenly", () => {
    expect(paginate(100, 2, 50)).toMatchObject({
      hasNext: false,
      pageCount: 2,
    });
  });
  ...
  it("counts a partial final page", () => {
    expect(paginate(101, 3, 50)).toMatchObject({
      hasNext: false,
      pageCount: 3,
    });
  });
