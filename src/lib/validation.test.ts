...
describe("validateEntry", () => {
  ...
  it("requires both fields", () => {
    ...
    expect(validateEntry("a@example.com", "   ").errors.message).toBe(
      "Message is required",
    );
  ...
  it("enforces length limits", () => {
    ...
    expect(
      validateEntry(`${longLocal}@example.com`, "hi").errors.email,
    ).toContain(String(EMAIL_MAX_LENGTH));
    expect(
      validateEntry("a@example.com", "x".repeat(MESSAGE_MAX_LENGTH)).valid,
    ).toBe(true);
    expect(
      validateEntry("a@example.com", "x".repeat(MESSAGE_MAX_LENGTH + 1)).errors
        .message,
    ).toBeDefined();
  });
  ...
  it("rejects control characters in an address", () => {
    for (const code of [0, 9, 0x1f, 0x7f]) {
      ...
      expect(validateEntry(email, "hi").errors.email).toBe(
  ...
  it("allows newlines but rejects other control characters", () => {
    ...
    expect(
      validateEntry("a@example.com", "null\u0000byte").errors.message,
    ).toBeDefined();
    expect(
      validateEntry("a@example.com", "tab\tseparated").errors.message,
    ).toBeDefined();
    expect(
      validateEntry("a@example.com", "cr\rreturn").errors.message,
    ).toBeDefined();
    expect(
      validateEntry("a@example.com", "del\u007fchar").errors.message,
    ).toBeDefined();
  });
